...
struct pid_list {
	pid_t pid; // individual pid
	struct list_head list; // reference to list_head where pid belongs
};


/* Store info about intercepted/replaced system calls */
typedef struct { // tracking the information of our system calls in a struct - individual syscall representation
	...
	int listcount;
	struct list_head my_list; // our LL of pids being monitored by this syscall
}mytable;

/* An entry for each system call in this "metadata" table */
mytable table[NR_syscalls]; // initializing our table (as defined above), this is nr_syscalls many table objects instanced

/* Access to the system call table and your metadata table must be synchronized */
spinlock_t my_table_lock = SPIN_LOCK_UNLOCKED; // spinlock for our bookkeeping table
spinlock_t sys_call_table_lock = SPIN_LOCK_UNLOCKED; // spinlock for the real table
//-------------------------------------------------------------
...
{
	// initalize a pid_list for pid being added
	struct pid_list *ple=(struct pid_list*)kmalloc(sizeof(struct pid_list), GFP_KERNEL);
	// if fails; error mem
	if (!ple)
		return -ENOMEM;
	// initialize the list that exists in the pid_list's attribute
	INIT_LIST_HEAD(&ple->list);
	// update our node's pid in the attribute
	ple->pid=pid;

	// add our newly made pid to the list
...
{
	// lock our table, remove all occurrences of current pid from our bookkeeping struct
	spin_lock(&my_table_lock);
	del_pid(current->pid);
	// unlock our table, call the original exit_group at the end
...
asmlinkage long interceptor(struct pt_regs reg) {
	// reg = struct that contains the registers required.

	long unsigned int syscall = reg.ax;
	int pid = current->pid;
	// 2 = all pids are monitored for syscall, 1 = make sure current pid is included in monitored list
	if (table[syscall].monitored == 2 || check_pid_monitored(syscall, pid) == 1) {
		log_message(pid, syscall, reg.bx, reg.cx, reg.dx, reg.si, reg.di, reg.bp);
	}

	// execute the original syscall at the end
...
asmlinkage long my_syscall(int cmd, int syscall, int pid) {
	...
	// syscall must be syscall <= NR_syscalls-1, != custom syscall
	if (syscall < 0 || syscall >= NR_syscalls || syscall == MY_CUSTOM_SYSCALL) {
		return -EINVAL;
	}

	// if statements branch for 4 commands
	if (cmd == REQUEST_SYSCALL_INTERCEPT) {
		// make sure the pid issuing this cmd is the root
		if(current_uid() != 0) {
			return -EPERM;
		}

		// this syscall is already intercepted, return EBUSY
		if (table[syscall].intercepted == 1) {
			return -EBUSY;
		}

		// we have passed the checks at this point, lock our table for changes
		spin_lock(&my_table_lock);
		// save the original syscall function, update intercept status, unlock
		table[syscall].f = sys_call_table[syscall];
		table[syscall].intercepted = 1;
		spin_unlock(&my_table_lock);

		// lock, set syscall-table to writable, replace the syscall with our interceptor
		spin_lock(&sys_call_table_lock);
		set_addr_rw((unsigned long) sys_call_table);
		sys_call_table[syscall] = interceptor;

		// set syscall-table to read-only, unlock
		...
		// this syscall is not yet intercepted, return EINVAL
		if (table[syscall].intercepted == 0) {
			return -EINVAL;
		}

		// we have passed the initial checks at this point
		// lock the syscall-table, set syscall-table to writable
		spin_lock(&sys_call_table_lock);
		set_addr_rw((unsigned long) sys_call_table);
		...
		// set syscall-table to read-only, unlock the syscall-table
		set_addr_ro((unsigned long) sys_call_table);
		spin_unlock(&sys_call_table_lock);

		// update our bookkeeping table by changing intercept status and clearing the pids monitored for it
		spin_lock(&my_table_lock);
		table[syscall].intercepted = 0;
		destroy_list(syscall);
		// unlock after we are done our changes
		spin_unlock(&my_table_lock);

		return 0;

	}
	// reach this code section if cmd is monitoring related, or invalid

	// pid must be positive/zero and existing
	...
	if (cmd == REQUEST_START_MONITORING) {
		// -EBUSY if pid is already monitored for specific or all pids
		if (check_pid_monitored(syscall, pid) == 1 || table[syscall].monitored == 2) {
			return -EBUSY;
		}

		// pid = 0 -> requested monitoring all pids for syscall
		if (pid == 0) { 
			if(current_uid() == 0) { // if they are root: lock, update monitored status, unlock
				spin_lock(&my_table_lock);
			...
			// make sure caller is either root or has same owner as pid being monitored
			if (current_uid() == 0 || check_pids_same_owner(current_uid(), pid) == 0) {
				spin_lock(&my_table_lock);
				// if monitoring fails due to memory, return -ENOMEM
				...
				// was not root, nor shared same owner
				return -EPERM;
			}
		}

	} else if (cmd == REQUEST_STOP_MONITORING) {
		// -EINVAL if pid given is not monitored yet
		// bonus not completed: if syscall is monitored for all pids and stop is requested, return invalid
		if (check_pid_monitored(syscall, pid) == 0 || table[syscall].monitored == 2) {
			return -EINVAL;
		}
		
		if (pid == 0) {
			// stop monitoring all if sender is root
			if (current_uid() == 0) {
				spin_lock(&my_table_lock);
			...
			if (current_uid() == 0 || check_pids_same_owner(current_uid(), pid) == 0) {
				// lock, remove the pid from monitor lists, unlock
				spin_lock(&my_table_lock);
				del_pid_sysc(pid, syscall);
				spin_unlock(&my_table_lock);
			} else { // was not root, nor shared same owner
				return -EPERM;
			}
		}

	} else {
		// the cmd given was not a valid input
		return -EINVAL;
	}
...
static int init_function(void) {
	...
	// to prevent 'mixed declaration' warning
	int i;
	// storing the originals before we make our changes
	orig_custom_syscall = sys_call_table[MY_CUSTOM_SYSCALL];
	orig_exit_group = sys_call_table[__NR_exit_group]; 

	//make syscalltable writable, lock it
	spin_lock(&sys_call_table_lock);
	set_addr_rw((unsigned long) sys_call_table);

	// replace custom_syscall, exit_group syscall.
	sys_call_table[MY_CUSTOM_SYSCALL] = my_syscall;
	sys_call_table[__NR_exit_group] = my_exit_group;

	//set syscalltable back to read-only
	set_addr_ro((unsigned long) sys_call_table);
	spin_unlock(&sys_call_table_lock);

	// initialize bookkeeping struct, lock it
	spin_lock(&my_table_lock);
	// establish a mytable object for each index of table; set attributes to default values, initialize the LL
	...
	// unlock after we are done instancing
	spin_unlock(&my_table_lock);
...
{    
	int i; // avoiding mixed declaration warning
	// make syscalltable writable, lock both tables
	spin_lock(&sys_call_table_lock);
	spin_lock(&my_table_lock);
	set_addr_rw((unsigned long) sys_call_table);
	...
	// restore exit_group syscall
	sys_call_table[__NR_exit_group] = orig_exit_group;

	// deintercept all applicable syscalls, cleanup all structs
	for (i = 0; i < NR_syscalls; i++) {
	...
	// set syscalltable back to read, unlock both our tables
	set_addr_ro((unsigned long) sys_call_table);
	spin_unlock(&sys_call_table_lock); 
	spin_unlock(&my_table_lock); 
}
